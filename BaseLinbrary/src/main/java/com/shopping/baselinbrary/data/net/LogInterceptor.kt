...
class LogInterceptor : Interceptor {
    ...
    private fun getHeadersString(headers: Headers?): String {
        ...
        while (i < size) {
            result.append("\t").append(headers.name(i)).append(": ").append(headers.value(i))
                .append(
                    "\n"
                )
            i++
    ...
    private fun getResponseData(response: Response?): String? {
        ...
        val charset: Charset = response.body()!!.contentType()?.charset(Charset.forName("UTF-8"))
